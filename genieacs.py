...
class Connection(object):
    ...
    def __normalize_device_id(self, device_id):
        """Normalize device ID - keep as-is, it's already in the correct format"""
        # Device IDs from GenieACS contain percent-encoded characters like %2D
        # We use them as-is without decoding
    ...
    def __request_post(self, url, data, conn_request=True):
        ...
        logger.info(f"POST Request URL: {request_url}")
        logger.info(f"POST Request Data: {json.dumps(data, indent=2) if data else 'None'}")
        
        try:
            r = self.session.post(request_url, json=data, timeout=self.timeout)
            logger.info(f"POST Response Status: {r.status_code}")
            logger.info(f"POST Response Text: {r.text[:500] if r.text else 'Empty'}")
            r.raise_for_status()
            ...
            logger.error(f"Connection Error in POST request: {e}")
            raise ConnectionError
    ...
    def device_get_by_id(self, device_id):
        ...
        device_id = self.__normalize_device_id(device_id)
    ...
    def device_delete(self, device_id):
        ...
        device_id = self.__normalize_device_id(device_id)
        self.__request_delete("/devices/" + requests.utils.quote(device_id, safe=''))
    ...
    def task_refresh_object(self, device_id, object_name, conn_request=True):
        ...
        try:
            return self.__request_post("/devices/" + requests.utils.quote(device_id, safe='') + "/tasks", data, conn_request)
    ...
    def task_set_parameter_values(self, device_id, parameter_values, conn_request=True):
        ...
        logger.info(f"task_set_parameter_values called with device_id: '{device_id}'")
        device_id = self.__normalize_device_id(device_id)
        logger.info(f"Using device_id: '{device_id}'")
        
        data = { "name": "setParameterValues",
                 "parameterValues": parameter_values }
        logger.info(f"Task data: {json.dumps(data, indent=2)}")
        
        try:
            # Use safe='' to encode all special characters including %
            encoded_id = requests.utils.quote(device_id, safe='')
            endpoint = "/devices/" + encoded_id + "/tasks"
            logger.info(f"Encoded device_id: '{encoded_id}'")
            logger.info(f"Full endpoint path: {endpoint}")
            return self.__request_post(endpoint, data, conn_request)
        except requests.exceptions.HTTPError:
            raise ItemNotFoundError
    ...
    def tag_assign(self, device_id, tag_name):
        ...
        device_id = self.__normalize_device_id(device_id)
        try:
            self.__request_post("/devices/" + requests.utils.quote(device_id, safe='') + "/tags/" + tag_name, None, False)
        except requests.exceptions.HTTPError:
            raise ItemNotFoundError
    ...
    def tag_remove(self, device_id, tag_name):
        ...
        device_id = self.__normalize_device_id(device_id)
        try:
            self.__request_delete("/devices/" + requests.utils.quote(device_id, safe='') + "/tags/" + tag_name)
        except requests.exceptions.HTTPError:
            raise ItemNotFoundError
